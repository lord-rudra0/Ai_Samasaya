...
from flask import Flask, request,session, render_template, jsonify, Response, redirect, url_for, send_from_directory, send_file, make_response, abort
import os
import pdfplumber
...
def handle_pdf():
    ...
    # Divide into chapters and generate questions and summaries
...
def check_answers():
    ...
    def validate_answer(question, answer):
        ...
        Please tell me whether this answer is correct or incorrect. 
        Return 'Correct' or 'Incorrect'.
        """
        
        response = model.generate_content(prompt)
        return response.text.strip()  # Return response after stripping any excess spaces
    
    # Iterate through user answers and validate them
    for chapter, data in user_answers.items():
        feedback_for_chapter = []
        for question_id, answer in data.items():
            # Retrieve question text from results
